...
def process():
    ...
    log.info("Query customer ids not downloaded.")
    user_ids = chat_dao.get_user_not_download()
    log.info("Customer ids: %s", repr(user_ids))
    if not user_ids:
        log.info("No customer need to download.")
        return
    config = LineBotConfig()
    log.info("Initial LineBotApi.")
    line_bot_api = LineBotApi(config.get_token())
    customer_list = []
    log.info("Start download customer profile.")
